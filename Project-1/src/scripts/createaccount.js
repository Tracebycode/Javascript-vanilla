...
export function  createaccount(params) {
    ...
    signupBtn.addEventListener('click',()=>{
        ...
        //handling signup with email and password
        if(signupform){
        signupform.addEventListener('submit',async(e)=>{
            e.preventDefault();
            const email = signupform['useremail'].value;
            const Passward = signupform['userpassward'].value;
           const success = await handlesignupwithemail(email, Passward);
    if (success) {
        
        parentcontainer.removeChild(overlay);
    } else {
        
        console.log("Signup failed. Modal remains open.");
    }
        })
        ...
        if(googlesignup){
            ...
            console.log("google signup button clicked");
              const suceess =  handlesignwithgoogle();
              if(suceess){
                parentcontainer.removeChild(overlay);
            }else{
                console.log("Google signup failed. Modal remains open.");
              }
            })
...
function createaccountform(){
    ...
    accountform.className="max-w-lg relative border-2 border-gray-300 bg-white shadow-md rounded-md items-center justify-center px-5 py-4 pt-8 pr-8";
    ...
    <form id="signupform">
     <div class="mb-2">
    <label for="username" class="block  text-sm font-bold text-[#1da1f2]">Enter Your Name</label>
              <input type="text" id="username"  name="username" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#1da1f2] focus:border-[#1da1f2] sm:text-sm" placeholder="Enter your name" required>
    </div >
    <div class="mb-2">
    ...
    <label for="Passward" class="block  text-sm font-bold text-[#1da1f2]">Enter you Passward</label>
     ...
     <div class="flex justify-center ">
              <button type="submit" class="text-white bg-[#1da1f2] hover:bg-[#1da1f2]/90 focus:ring-4 focus:outline-none focus:ring-[#1da1f2]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#1da1f2]/55 me-2 mb-2">Create Account</button> 
      </div>
    ...
    <div>  
      <button id="googlesignup" class="flex items-center bg-white dark:bg-gray-900 border border-gray-300 rounded-lg shadow-md px-6 py-2 text-sm font-medium text-gray-800 dark:text-white hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
